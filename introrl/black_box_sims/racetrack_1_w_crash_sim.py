...
class RaceTrack_1_w_Crash( Simulation ):
    
    def __init__(self, name='RaceTrack_1_w_Crash Simulation', s_hash_rowL=s_hash_rowL, 
        ...
        terminalL = [('Done','Done',0,0),('Crash','Crash',0,0)] # terminal state hashes.
    ...
    def get_action_snext_reward(self, s_hash, a_desc):
        ...
        if (vx,vy)==(0,0) and a_desc==(0,0):
            print('ERROR... a_desc==(0,0) when (vx,vy)==(0,0), s_hash=', s_hash, ' a_desc=', a_desc)
        ...
        if (x2,y2) in self.racetrack_area:
            ...
            if y2>=16 and (x2>=26 and x2<=31):
                ...
                reward = 1000 # provide big incentive to cross finish line
                #print('Done', s_hash, a_desc, end='')
            else:
                sn_hash = ('Crash','Crash',0,0)
                reward = -1000 # provide big penalty for crashing
    ...
    def plot_policy(self, ax, policy_obj ):
        ...
        for start_state in self.limited_start_state_list():
            ...
            while s_hash in policyD:
                ...
                sn_hash = (x2,y2, vx2,vy2)
                print('from ',s_hash,' action ',a_desc,' leads to ',sn_hash)
                s_hash = sn_hash
                (x,y, vx,vy) = s_hash
                xL.append( x )
                yL.append( y )
