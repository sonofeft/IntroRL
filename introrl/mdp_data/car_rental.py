...
def fill_prob_lookupD(lam=3):
    for n in range(POISSON_UPPER_BOUND + 1 ):
...
def get_prob_reward( s1, s2, a_desc):
    ...
    for n1_rent_request in range( POISSON_UPPER_BOUND ):
        for n2_rent_request in range( POISSON_UPPER_BOUND ):
            ...
            # need to consider variable returns at each site changing next state
            for n1_rtns_poi in range( POISSON_UPPER_BOUND ):
                for n2_rtns_poi in range( POISSON_UPPER_BOUND ):
                    
                    prob_rtnd = prob_lookupD[ (n1_rtns_poi, N1_RTNS) ] \
                                * prob_lookupD[ (n2_rtns_poi, N2_RTNS) ]
                    
                    # next state
                    sn1 = int(min(n1 - actual_n1_rented + n1_rtns_poi, MAX_CARS))
                    sn2 = int(min(n2 - actual_n2_rented + n2_rtns_poi, MAX_CARS))
                    
                    add_results( s1, s2, a_desc, prob_rented*prob_rtnd, reward, sn1, sn2 )
...
def get_env():
    ...
    env = EnvBaseline( name="Jacks Car Rental (var rtn)" ) # GenericLayout set below
