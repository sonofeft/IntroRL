...
for Nsteps in nstepL:
    nstep_walkerL.append( NStepTDWalker(rw_mrp, Nsteps=Nsteps,  policy=policy) )
    for alpha in alphaL:
...
for loop in range(AVE_OVER): # average rms curves over AVE_OVER runs
    ...
    for _ in range(10):
        for istep,Nsteps in enumerate( nstepL ):
            for alpha in alphaL:
                nstep_walkerL[ istep ].do_td_state_value_updates( sv_collD[ (alpha, Nsteps) ], 
...
ax.legend()
ax.set(title='Figure 7.2 Gamma = %g, Ave Over %i Runs\n'%(GAMMA, AVE_OVER) + rw_mrp.info  )
#ax.axhline(y=0, color='k')
#ax.axvline(x=0, color='k')
fig.savefig("figure_7_2_random_walk_19_pi.png")
plt.show()
