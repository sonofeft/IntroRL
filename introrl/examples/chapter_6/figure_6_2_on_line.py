...
ax.plot(mc_rmsL, 'r-', label='MC, alpha=%g'%alpha_mc)
ax.plot(td_rmsL, 'c-', label='TD(0), alpha=%g'%alpha_td)
...
mc_erros = [0.32090206, 0.3073737 , 0.28464613, 0.24484697, 0.20757003,
       ...
       0.05960022]
ax.plot(mc_erros, 'r:', label='np-MC, batch')
ax.plot(td_erros, 'c:', label='np-TD(0), batch')

ax.legend()
ax.set(title='Figure 6.2 NON-Batch MC & TD(0) Random Walk')
ax.axhline(y=0, color='k')
ax.axvline(x=0, color='k')
plt.ylabel('Ave. RMS Error (100 experiments)')
plt.xlabel('Walks / Episodes')
fig.savefig("fig_6_2_online_random_walk.png")
plt.show()
