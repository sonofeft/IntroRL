...
pickle_fname = 'fig_6_3_interim_data.pickle'
RUN_COUNT = 50  # Sutton & Barto call for 50000 runs.
NUM_EPISODES = 100
        ...
        for alpha in ALPHA_LIST:
            ...
            policy, state_value = \
                expected_sarsa_eps_greedy( CW,  learn_tracker=learn_tracker,
                                      ...
                                      show_banner = False,
                                      max_num_episodes=NUM_EPISODES, min_num_episodes=NUM_EPISODES, max_abserr=0.000001, 
                                      gamma=1.0,
            ...
            ExpSarsa_raveD[alpha][0].add_val( sum(reward_sum_per_episodeL)/len(reward_sum_per_episodeL) )
            ExpSarsa_raveD[alpha][1].add_val( sum(reward_sum_per_episodeL)/100.0 )
...
def get__sarsa_data():
    ...
    for loop in range(Nruns, RUN_COUNT):
        for alpha in ALPHA_LIST:
            ...
            ave_run_time.add_val( time.time() - start_time ) # compute average run time
            Sarsa_raveD[alpha][0].add_val( sum(reward_sum_per_episodeL)/len(reward_sum_per_episodeL) )
            Sarsa_raveD[alpha][1].add_val( sum(reward_sum_per_episodeL)/100.0 )
...
def get_qlearning_data():
    ...
    for loop in range(Nruns, RUN_COUNT):
        for alpha in ALPHA_LIST:
            ...
            ave_run_time.add_val( time.time() - start_time ) # compute average run time
            Qlearn_raveD[alpha][0].add_val( sum(reward_sum_per_episodeL)/len(reward_sum_per_episodeL) )
            Qlearn_raveD[alpha][1].add_val( sum(reward_sum_per_episodeL)/100.0 )
