...
class PartitionedSegment( object ):
    
    def __init__(self, lo_val=0.0, hi_val=1.0, num_regions=10):
        """
        Break lo to hi val into num_regions.
        NOTE: <lo_val or >hi_val is OUTSIDE the range.
        The number of bounded regions is therefore num_regions
        """
        self.lo_val = lo_val
        self.hi_val = hi_val
        ...
        if num_regions <= 0:
            raise ValueError( 'num_regions MUST be > 0. Input value =',num_regions )
            
        self.step = float(hi_val - lo_val) / float(num_regions)
        
        self.edgeL = [self.lo_val + i*self.step for i in range(num_regions+1)]
        
    def get_region(self, val):
        """region 0 is < lo_val, region num_regions-1 is >= hi_val."""
        if val < self.lo_val:
            return -1
        elif val > self.hi_val:
            return -1
        else:
            i = min(self.max_region, floor( (val-self.lo_val) / self.step ))
            return i
...
class Tile( object ):
    """
    Some number of PartitionedSegment objects to define k dimensional space.
    """
...
def plot_tile(plt, tile, color='r', linestyle='-', n=0 ):
    ...
    for i in range(d2.num_regions+1):
        y = y1 + i*d2.step
        plt.plot([x1,x2],[y,y], color=color, linestyle=linestyle)
        
    for i in range(d1.num_regions+1):
        x = x1 + i*d1.step
        plt.plot([x,x],[y1,y2], color=color, linestyle=linestyle)
            

if __name__=="__main__":
    import matplotlib.pyplot as plt
    
    d = PartitionedSegment(lo_val=0.0, hi_val=1.0, num_regions=4)
    if 1:
        for val in [-99,-.1, 0.0, .1, 0.5, .9, 1.0, 1.1]:
            print('%8g %i'%(val, d.get_region(val)), end=' ')
            print( d.get_numpy_encoding(val))
    print('-'*66)
    ...
    if 1:
        ...
        for itile, tile in enumerate(T.tileL):
            print(itile,'Tile')
            plot_tile(plt, tile, color=colorL[itile%len(colorL)], 
